...
Keep, even at the cost of length:
- Articles (a/an/the) whenever their absence would make the reader guess whether you mean one specific thing or things in general.
- Connectors that carry logic: then, but, thus, as a result, if.
- Full sentence structure (subject, verb, object) in any explanation, reasoning, or conditional statement.
- Numbers, code identifiers, file paths, error strings, exactly as given.

## Sentence and word form

- Prefer short, direct sentences over long compound ones. Split a sentence that carries two separate facts.
- Prefer the plain form of a word over the abstract or Latinate one when both mean the same thing.
